...
public class MenuActivity extends AppCompatActivity implements ConnectionStatusListener {
    ...
    ImageView btnDualDPadAccess;
    TextView textViewDualDPadAccess;

    TextView freeGameTextView;
    TextView geographyGameTextView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Setting the chalkboard se font to the main TextViews
        freeGameTextView = (TextView) findViewById(R.id.free_game);
        geographyGameTextView = (TextView) findViewById(R.id.geography_game);
        deviceInformationTextView = (TextView) findViewById(R.id.device_info);
        controllerTextView = (TextView) findViewById(R.id.controller_text_view);
        Typeface type = Typeface.createFromAsset(getAssets(),"fonts/chalkboardseregular.ttf");
        freeGameTextView.setTypeface(type);
        geographyGameTextView.setTypeface(type);
        deviceInformationTextView.setTypeface(type);
        controllerTextView.setTypeface(type);
        ...
        // Set the button for the Dual D-Pad as invisible
        btnDualDPadAccess = (ImageView) findViewById(R.id.btn_controller);
        textViewDualDPadAccess = (TextView) findViewById(R.id.controller_text_view);
    ...
    public void onDemoSelected(View view) {
        ...
        switch (iw.getId()) {
            ...
            case R.id.btn_information:
                ...
                break;
            // EVENTSERVICE_SERVICE_UUID (as the "GamepadControllerActivity") --> implements the "FreeGameActivity" ----------------------------------
            case R.id.btn_temperature_alarm:
                ...
                if (MicroBit.getInstance().hasService(BleAdapterService.EVENTSERVICE_SERVICE_UUID)) {
                    intent = new Intent(MenuActivity.this, FreeGameActivity.class);
                    intent.putExtra("MyBoolean", DebugModeOn);
                    startActivity(intent);
                } else {
                    showMsg(Utility.htmlColorRed("Event Service not on this micro:bit"));
                }
                break;
            // EVENTSERVICE_SERVICE_UUID (as the "GamepadControllerActivity") --> implements the "GeographyGameActivity" -----------------------------
            case R.id.btn_squirrel_counter:
                Log.d(Constants.TAG, "onDemoSelected Squirrel Counter");
                if (MicroBit.getInstance().hasService(BleAdapterService.EVENTSERVICE_SERVICE_UUID)) {
                    intent = new Intent(MenuActivity.this, GeographyActivity.class);
                    intent.putExtra("MyBoolean", DebugModeOn);
                    startActivity(intent);
                } else {
                    showMsg(Utility.htmlColorRed("Event Service not on this micro:bit"));
                ...
                break;
            // This is the activity to use to remotely control the Bit:Buggy Car ---------------------------------------------------------------------
            case R.id.btn_controller:
                Log.d(Constants.TAG, "onDemoSelected Controller");
                // /!\ Trick to make the button for the controller available only if we click 5 times right under "Device Information"
                count += 1;
                if (count > 4) {
                    btnDualDPadAccess.setImageResource(R.drawable.controller);
                    textViewDualDPadAccess.setText(getString(R.string.controller));
                }
                if (count > 5 & MicroBit.getInstance().hasService(BleAdapterService.EVENTSERVICE_SERVICE_UUID)) {
