...
//        showResult(copy, "あ.B.2.c) corners", 0, 0);
        ...
        Rect roi_probe_area = new Rect(5*o_m, 55*o_m, (15-5)*o_m, 15*o_m); // Rect roi_probe_area = new Rect(5*o_m, 60*o_m, (15-5)*o_m, 10*o_m);
...
//        boolean black = Core.mean(probe_area_thresh).val[0] < Math.round((maxVal_probe_area_thresh - minVal_probe_area_thresh)/2);
        //-----
        boolean black = (Core.mean(probe_area).val[0]+ Core.mean(probe_area).val[1]+ Core.mean(probe_area).val[2])/3 < 120; // 100
        if (black) {
